...
class SigmoidHiddenUnit(HiddenUnit):
    ...
    def activation(self, acc_sum):
        with np.errstate(divide='ignore', over='ignore', under='ignore'):
            return np.where((np.abs(acc_sum) <= 15), (1.0 / (1.0 + np.exp(-acc_sum))) - 0.5, np.sign(
                acc_sum) * 0.5)
...
class AsigmoidHiddenUnit(HiddenUnit):
    ...
    def activation(self, acc_sum):
        with np.errstate(divide='ignore', over='ignore', under='ignore'):
            return np.where((np.abs(acc_sum) <= 15), (1.0 / (1.0 + np.exp(-acc_sum))), \
                               np.sign(acc_sum) * 0.5 + 0.5)
...
class GaussianHiddenUnit(HiddenUnit):
    ...
    def activation(self, acc_sum):
        with np.errstate(divide='ignore', over='ignore', under='ignore'):
            tmp = -0.5 * acc_sum * acc_sum
            return np.where((tmp < -75.0), np.zeros(tmp.shape()), np.exp(tmp))
